...
ssh $PROD_VPS_USER@$PROD_VPS_IP "docker exec $CONTAINER_NAME rm -rf /app/app/*"

# 2. Копируем новые файлы на VPS
echo "📤 Копирование файлов на VPS..."
ssh $PROD_VPS_USER@$PROD_VPS_IP "mkdir -p /tmp/client-deploy"
scp -r ./app/* $PROD_VPS_USER@$PROD_VPS_IP:/tmp/client-deploy/

# 3. Копируем файлы в контейнер
echo "📦 Копирование файлов в контейнер $CONTAINER_NAME:/app/app/..."
ssh $PROD_VPS_USER@$PROD_VPS_IP "docker cp /tmp/client-deploy/. $CONTAINER_NAME:/app/app/"

# 4. Очистка временной папки на VPS
ssh $PROD_VPS_USER@$PROD_VPS_IP "rm -rf /tmp/client-deploy"

# 5. Проверка
echo "📋 Проверка файлов внутри контейнера:"
ssh $PROD_VPS_USER@$PROD_VPS_IP "docker exec $CONTAINER_NAME ls -la /app/app/ | head -15"
